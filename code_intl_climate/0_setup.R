library(utils) # can't install on CodeOcean
...
# On ARM Mac, if this doesn't work (runs infinitely). Download the archive and from the Terminal run `R.exe CMD INSTALL plotly.R-4.9.4.1.tar.gz'
# package("reticulate")
if (!is.element("gdata", installed.packages()[,1])) package("memisc")
package('gdata') # If it bugs, install it from the command line: Rscript -e "install.packages('gdata', repos='https://cran.rstudio.com/')"
# if (!is.element("prediction", installed.packages()[,1])) install.packages("https://cran.r-project.org/src/contrib/Archive/prediction/prediction_0.3.17.tar.gz", repos = NULL, type = "source")
# if (!is.element("margins", installed.packages()[,1])) install.packages("https://cran.r-project.org/src/contrib/Archive/margins/margins_0.3.26.tar.gz", repos = NULL, type = "source")
# library(prediction)
# library(margins)
# package("reticulate")
package("Hmisc")
...
# package("descr")
# package("clipr")
package("ergm") # wtd.median Rscript -e "install.packages('ergm', repos='https://cran.rstudio.com/')"
package("mfx") # imports lmfest hence coeftest
# package("plotrix")
# package("grDevices") # can't install on CodeOcean
# package("colorspace")
...
# package("XML")
# package("equivalence")
package("RMallow") # AllSeqDists, list distances
# package("rpart")
# package("readr")
# package("caTools")
package("dplyr")
...
package("broom")
package("tidytext")
package("modelsummary", version = "1.4.1")
...
if (!is.element("memisc", installed.packages()[,1]) || packageVersion("memisc")!="0.99.22") {
  tryCatch({  detach("package:memisc", unload = TRUE)}, error = function(cond) { print("This is just a warning: memisc not loaded yet.") } )
  tryCatch({  remove.packages("memisc")}, error = function(cond) { print("This is just a warning: memisc not found.") } )
...
library(memisc)
# To fix the bug with is.na() on R 4.2, code below from https://gist.github.com/alexeyknorre/b0780836f4cec04d41a863a683f91b53
...
library(stargazer)
package("boot") # Reverse IV
package("pbapply") # Reverse IV
# package("htmlwidgets")
# package("ggeffects")
# package("magick") # Bug sur Ubuntu, ne surtout pas décommenter sur Ubuntu
# library(magick) 

d <- function(str, data = all) { # data from name
  if (exists(tolower(str)) && !any(grepl("function", class(eval(str2expression(tolower(str))))))) return(eval(str2expression(tolower(str)))) 
  else return(data[data$country == str,])
}
...
weighted_cor <- function(x, y, weights) {
  ...
  weighted_mean_x <- sum(weights * x) / sum(weights)
  weighted_mean_y <- sum(weights * y) / sum(weights)
  
  # Calculate the weighted covariance
  weighted_covariance <- sum(weights * (x - weighted_mean_x) * (y - weighted_mean_y)) / sum(weights)
  
  # Calculate the weighted variances
  weighted_variance_x <- sum(weights * (x - weighted_mean_x)^2) / sum(weights)
  weighted_variance_y <- sum(weights * (y - weighted_mean_y)^2) / sum(weights)
  
  # Calculate the weighted correlation
  weighted_correlation <- weighted_covariance / sqrt(weighted_variance_x * weighted_variance_y)
...
decrit <- function(variable, data = e, miss = TRUE, weights = NULL, numbers = FALSE, which = NULL, weight = T) { 
  ...
  if (weight) {
    if (missing(weights)) weights <- data[["weight"]]  
  ...
  if (length(annotation(variable))>0 & !numbers) {
    if (!miss) {
      if (length(which(!is.na(suppressWarnings(as.numeric(levels(as.factor(variable)))))))==0) { describe(as.factor(variable[no.na(variable)!=""]), weights = weights[no.na(variable)!=""], descript=Label(variable)) } 
      else { describe(as.numeric(as.vector(variable[no.na(variable)!=""])), weights = weights[no.na(variable)!=""], descript=Label(variable)) } 
    }
    else {
      if (length(which(suppressWarnings(!is.na(as.numeric(levels(as.factor(variable)))))))>10) describe(include.missings(variable[no.na(variable)!="" & !is.na(variable)]), weights = weights[no.na(variable)!="" & !is.na(variable)], descript=Label(variable)) 
...
Levels <- function(variable, data = e, miss = TRUE, numbers = FALSE, values = TRUE, concatenate = FALSE) {
  if (values) {
    ...
    else if (is.character(variable)) Levs <- if (length(unique(variable)) > 13) "[string variable]" else sort(as.character(unique(variable)))
...
reg_lm <- function(dep_var = NULL, indep_vars = NULL, formula = NULL, df = e, weights = "weight", summary = FALSE, weight = T) {
  if (is.null(formula)) formula <- reg_formula(dep_var, indep_vars)
  df$w <- df[[weights]]
  model <- if (weight) lm(formula, data = df, weights = w) else lm(formula, data = df)
  if (summary) return(summary(model)) else return(model)
}
desc_table <- function(dep_vars, filename = NULL, data = e, indep_vars = control_variables, indep_labels = NULL, weights = data$weight, add_lines = NULL, model.numbers = T, stars = FALSE, #!mean_above,
                       save_folder = "../tables/", dep.var.labels = NULL, dep.var.caption = c(""), digits= 3, mean_control = FALSE, logit = FALSE, atmean = T, robust_SE = T, omit = c("Constant", "Gender: Other", "Economic Leaning: PNR"),
  ...
  table <- do.call(stargazer, c(models, list(out=file_path, header=F, model.numbers = model.numbers,
                                             ...
                                             coef = coefs, se = SEs, star.cutoffs = if (stars) NULL else NA, 
  ...
  if (!nolabel) table <- table_mean_lines_save(table, mean_above = mean_above, only_mean = only_mean, indep_labels = indep_labels, indep_vars = indep_vars, add_lines = add_lines, file_path = file_path, oecd_latex = FALSE, nb_columns = length(indep_vars_included), omit = omit)
  return(table)
}
...
table_mean_lines_save <- function(table, mean_above = T, only_mean = FALSE, indep_vars = NULL, indep_labels = indep_vars, add_lines = NULL, file_path = NULL, oecd_latex = FALSE, nb_columns = 2, omit = c("Constant", "Gender: Other", "econ_leaningPNR")) {
  if (mean_above) {
    ...
    first_lab <- ifelse(missing(indep_labels), latexify(indep_vars[1]), paste0(latexify(indep_labels[1]), " &")) 
...
covariates_with_several_values <- function(data, covariates) { 
  several_values <- c()
...
same_reg_subsamples <- function(dep.var, dep.var.caption = NULL, covariates = setA, data = all, along = "country3", along.levels = Levels(along, data), covariate.labels = NULL, nolabel = FALSE, include.total = FALSE, add_lines = NULL, mean_above = T, only_mean = FALSE, mean_control = T, stars = FALSE,
                                dep.var.label = dep.var, logit = FALSE, robust_SE = T, atmean = T, keep = NULL, omit = c("Constant", "Gender: Other", "Economic Leaning: PNR"), print_regs = FALSE, no.space = T, filename = dep.var, folder = "../tables/regs_countries/", digits= 3, model.numbers = T, replace_endAB = NULL) {  
  ...
  table <- do.call(stargazer, c(if (include.total) models else models[-1], list(out=NULL, header=F, model.numbers = model.numbers, covariate.labels = covariate_labels, coef = if (include.total) coefs else coefs[-1], se = if (include.total) SEs else SEs[-1], add.lines =list(c(mean_text, means)), 
                                                                                dep.var.labels = if (include.total) c("All", along.levels) else along.levels, dep.var.caption = dep.var.caption, multicolumn = F, float = F, keep.stat = c("n", "rsq"), omit.table.layout = "n", star.cutoffs = if (stars) NULL else NA, keep=keep, omit = omit, no.space = no.space)))
...
create_covariate_labels <- function(coefs_names, regressors_names = regressors_names, keep = NULL, omit = "Constant") {
  ...
  covariate_labels <- unique(covariate_labels[c(which(!grepl("[:*]", names(covariate_labels))), which(grepl("[:*]", names(covariate_labels))))]) 
...
stack_bars <- function(vars, data=s, miss=T, labels=NA, title=NA, accord=FALSE, en = FALSE, margin=c(2.5,17,0,3), cex=1, width=0.77/length(vars), weights=FALSE) {
  matrice <- c()
  colors <-   c(rainbow(4, end=4/15)[1:3], "green", "forestgreen") 
  for (var in vars) {
    if (miss) {
...
oui_non <- function(vars, file, labels = vars, data = s, display_value = T, sort=T, colors=color(2), weights=T, margin_r=0, margin_l=NA, title="", en=FALSE, NSP=FALSE) { # 250 l
  ...
  y <- c(y, '')
  bars <- plot_ly(data, x = ~o, y = ~y, type = 'bar', orientation = 'h', text = hover_oui, textposition = 'auto', 
...
data5 <- function(vars, data=e, miss=T, weights=T, rev=FALSE) {
  matrice <- c()
  colors <-  c(rainbow(4, end=4/15), "forestgreen") 
  for (var in vars) {
    if (miss) {
...
dataN <- function(var, data=e, miss=T, weights = T, return = "", fr=F, rev=FALSE, rev_legend = FALSE) {
  missing_labels <- c("NSP", "PNR", "Non concerné·e", "Included", "Don't know", "PNR or other", "NSP ou autre", "PNR ou autre")
  if (is.null(data[['weight']])) weights <- F 
  mat <- c()
  if (is.character(data[[var]]) | (is.numeric(data[[var]])) | is.logical(data[[var]])) v <- as.factor(data[[var]]) 
  else v <- data[[var]]
  if (setequal(levels(v), c(T, F))) levels <- c(T) 
  ...
  if (weights) N <- sum(data[['weight']][!is.missing(v) & (!(v %in% missing_labels))]) 
  else N <- length(which(!is.missing(v) & (!(v %in% missing_labels)))) 
  for (val in levels) {
  ...
  if (miss) {
    if (is.null(annotation(v))) {
      if (weights) mat <- c(mat, sum(data[['weight']][which(is.na(v) | v %in% missing_labels)])/N) 
      else mat <- c(mat, length(which(is.na(v) | v %in% missing_labels))/N) 
    } else  {
      if (weights) mat <- c(mat, sum(data[['weight']][which(is.missing(v) & !is.na(v))])/N) 
      else mat <- c(mat, length(which(is.missing(v) & !is.na(v)))/N) } } 
...
barres12 <- function(vars, df=list(e, e2), labels, legend=hover, comp = "V2", orig = NULL, miss=T, weights = T, fr=F, rev=T, color=c(), rev_color = FALSE, hover=legend, sort=TRUE, thin=T, return="", showLegend=T, export_xls = F) {
  ...
  if (missing(legend) & missing(hover)) {
    if (is.logical(df[[1]][[vars[1]]])) hover <- legend <- labels 
...
barresN <- function(vars, along = NULL, df=list(e), labels = NULL, legend=hover, miss=T, weights = T, fr=F, rev=T, color=c(),  use_plotly = T,
  ...
  data1 <- dataKN(vars, data=df[[1]], miss=miss, weights = weights, return = "", fr=fr, rev=rev)
  if (missing(legend) & missing(hover)) {
    if (is.logical(df[[1]][[vars[1]]])) { showLegend = F; legend <- "True"; hover <- labels; } 
    else hover <- legend <- dataN(var = vars[1], data=df[[1]], miss=miss, weights = weights, return = "legend", fr=fr, rev_legend = rev) }
  agree <- order_agree(data = data1, miss = miss)
  if (is.logical(df[[1]][[vars[1]]])) agree <- rev(agree)
  if (return=="levels") {if (is.null(levels)) {return(labels)} else {return(levels)}}
  else if (return=="labels") return(labels) 
  else if (return=="legend") return(legend)
  else {
    ...
    if (return=="data") { return(plotted_data)
      ...
      return(barres(data = plotted_data, labels=labels[not_nan], legend=legend, 
...
color5 <- c(rainbow(4, end=4/15)[1:3], "#00FF00", "#228B22") 
...
barres <- function(data, vars, file, title="", labels, color=c(), rev_color = FALSE, hover=legend, nsp=TRUE, sort=TRUE, legend=hover, showLegend=T, margin_r=0, margin_l=NA, online=FALSE, use_plotly = T, export_xls = F, digits = 0,
  ...
  if (missing(data) & !missing(vars)) {
    ...
    if (missing(legend) & missing(hover)) {
      if (is.logical(df[[vars[1]]])) hover <- legend <- labels 
  ...
  legendSize <- 15+2
  legendY <- 1.1  + 0.3*thin/(ncol(data)-1) 
  legendX <- 0.2
  legendFont = font #'Open Sans'
  if (is.na(margin_l)) { margin_l <- 4.7*max(nchar(labels)/(1 + str_count(labels, '<br>'))) }
  if (max(nchar(labels)) > 25) { legendSize <- 15 } 
  ...
  if (nrow(data)==1 & (sort | !showLegend)) {  
  ...
  if (use_plotly) {
    bars <- plot_ly(x = data[1,], y = labels, type = 'bar', orientation = 'h', text = values[,1], textposition = 'auto', 
                    error_x = list(visible = error_margin, array=qnorm(1-0.05/2)*sqrt(data[1,]*(1-data[1,])/(N-1)), color = color_margin), 
                    hoverinfo = hovers[,1], name=legend[1], marker = list(color = color[1], line = list(color = 'white'))) %>% 
      
      plotly::layout(xaxis = list(title = "",
                                  showgrid = show_ticks,
                                  showline = FALSE,
                                  showticklabels = show_ticks,
                                  gridcolor = toRGB("gray70"), 
                                  gridwidth = 1,
                                  griddash = "dot",
                                  autotick = FALSE,
                                  ticks = "outside",
      ...
      margin = list(l = margin_l, r = margin_r, t = margin_t, b = 24, autoexpand = thin), 
      legend = list(orientation='h', y=legendY, x=legendX, traceorder='normal', font=list(size=legendSize+2, color='black', family = font)), 
...
save_plot <- function(plot=NULL, filename = deparse(substitute(plot)), folder = '../figures/', width = dev.size('px')[1], height = dev.size('px')[2], method='dev', trim = T, format = 'png') {
  if (any(class(plot) %in% c("data.frame", "array"))) {
    ...
    } # to crop pdf, see also code_oecd/crop_pdf.sh and run it in the desired folder
}
...
heatmap_table <- function(vars, labels = vars, data = all, along = "country_name", special = c(), conditions = c("> 0"), on_control = T, alphabetical = FALSE, export_xls = T, filename = "", sort = FALSE, weights = "weight") {
  ...
  if (along == "country_name" & !alphabetical & exists("countries_names")) {
    ...
    levels <- c()
    for (l in names) if (l %in% unique(e[[along]])) levels <- c(levels, l)
  ...
  for (c in up_labels) {
    ...
    else weight_var <- weights
    for (v in 1:nb_vars) {
      var_c <- df_c[[vars[v]]][!is.na(df_c[[vars[v]]])]
      if (length(var_c) > 0) table[v,c] <- eval(str2expression(paste0("wtd.mean(var_c", conditions[v], ", na.rm = T, weights = df_c$", weight_var, "[!is.na(df_c[[vars[v]]])])")))
    }
  }
  row.names(table) <- labels
  if (sort) table <- table[order(-table[,1]),]
  if (export_xls) save_plot(table, filename = sub("figures", "xlsx", paste0(folder, filename, replacement_text)))
  return(table)
}
heatmap_wrapper <- function(vars, labels = vars, name = deparse(substitute(vars)), along = "country_name", labels_along = NULL, special = c(), conditions = c("> 0"), df = all, width = NULL, height = NULL, alphabetical = T, on_control = T, export_xls = T, format = 'pdf', sort = FALSE, proportion = NULL, percent = FALSE, trim = T, colors = 'RdYlBu', weights = "weight") {
  ...
  for (cond in conditions) {
    ...
    tryCatch({
      if (cond %in% c("/", "-")) { 
        pos <- heatmap_table(vars = vars, labels = labels, data = df, along = along, special = special, conditions = ">= 1", on_control = on_control, alphabetical = alphabetical, sort = FALSE, weights = weights)
        neg <- heatmap_table(vars = vars, labels = labels, data = df, along = along, special = special, conditions = "<= -1", on_control = on_control, alphabetical = alphabetical, sort = FALSE, weights = weights)
        if (cond == "-") temp <- pos - neg else temp <- pos / (pos + neg)
        for (i in 1:length(vars)) if (is.logical(df[[vars[i]]])) temp[i, ] <- pos[i, ]
      } else {  temp <- heatmap_table(vars = vars, labels = labels, data = df, along = along, special = special, conditions = cond, on_control = on_control, alphabetical = alphabetical, sort = FALSE, weights = weights) }
      if (!missing(labels_along) & length(labels_along) == ncol(temp)) colnames(temp) <- labels_along
      if (sort) temp <- temp[order(-temp[,1]),]
      if (export_xls) save_plot(as.data.frame(temp), folder = "", filename = sub("figures", "xlsx", paste0(folder, filename, replacement_text)))
      heatmap_plot(temp, proportion = ifelse(missing(proportion), cond != "", proportion), percent = percent, colors = colors)
      save_plot(filename = paste0(folder, filename, replacement_text), folder = "", width = width, height = height, format = format, trim = trim)
...
mean_ci <- function(along, outcome_vars = outcomes, outcomes = paste0(outcome_vars, conditions), covariates = NULL, subsamples = NULL, conditions = c(" > 0"), invert_y_along = FALSE, df = e, labels = outcome_vars, factor_along = FALSE,
  ...
  if (!is.null(covariates)) { # If conditional (regressions)
    ...
    if (!is.null(subsamples)) { # Configuration a.
      ...
      if (missing(labels) | labels == outcome_vars) labels <- y_loop 
...
plot_along <- function(along, mean_ci = NULL, vars = outcomes, outcomes = paste0(vars, conditions), covariates = NULL, subsamples = NULL, conditions = c(" > 0"), invert_y_along = FALSE, df = e, labels = vars, factor_along = FALSE,
  ...
  plot
  if (save) save_plotly(plot, filename = name, folder = folder, width = width, height = height, trim = T, method = "xlsx")
...
update_constant <- function(data = all, Country = NULL, control_group = FALSE) {
  if (control_group) df <<- e <<- data[data$treatment == "None", ]
  else df <<- e <<- data
  if (!missing(Country)) country <<- Country
  else country <<- toupper(deparse(substitute(data)))
  if (country %in% c("ALL", "all")) {
  ...
  alphabetical <<- FALSE
  heatmap_conditions <<- c("", ">= 1", "/") 
  along <<- "country_name"
  nb_levels <<- if (!is.null(Country) && Country == "alla") 20 else length(Levels(e[[along]]))
  cond <<- ">= 1"
  parentheses <<- FALSE
  nolabel <<- T
  on_control <<- T
  export_xls <<- F
  special <<- c("High-income", "Middle-income")
  save_plotly_new_filename <<- function(plot, filename = NULL, width = dev.size('px')[1], height = dev.size('px')[2], method='orca', trim = T) {
    ...
    return(save_plotly(plot, filename = filename, folder = folder, width = width, height = height, method=method, trim = trim, format = 'pdf'))
  }
}
