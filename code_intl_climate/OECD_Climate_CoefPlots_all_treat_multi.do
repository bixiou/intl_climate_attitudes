...
cap program drop graphcoefplots_treat_multi

program define graphcoefplots_treat_multi
	* Arguments 
	...
	* Outcomes Labels
	...
	if strpos("``i''", "index_tax_emissions_plus") loc ``i''t "Believes the policy would reduce GHG emissions"
	if strpos("``i''", "index_tax_pollution") loc ``i''t "Believes the policy would reduce air pollution"
	...
	if strpos("``i''", "index_standard_em_plus") loc ``i''t "Believes the policy would reduce air pollution"
	if strpos("``i''", "index_standard_pollution") loc ``i''t "Believes the policy would reduce GHG emissions"
	...
	if strpos("``i''", "index_invest_em_plus") loc ``i''t "Believes the policy would reduce air pollution"
	if strpos("``i''", "index_invest_pollution") loc ``i''t "Believes the policy would reduce GHG emissions"
	...
	if strpos("``i''", "donation_binary") loc ``i''t "Willing to donate to reforestation cause"
	if strpos("``i''", "donation_pct") loc ``i''t "% of prize willing to donate to reforestation cause"
	...
	*----------------------------		
	...
	if "`2'" != "all_mech" {
		...
		if "$gname" == "support_main_trans" & `i' > 4 {
			loc toplot `toplot' (``i''A, keep(2.treatment) rename(2.treatment = "```i''t'") offset(0.06) mlcolor("${p2}%0") $dotsize ms(`sym1') mfcolor("${p2}%0") ciopts( lwidth($ciwidth )  color("${p2}%0") ) )
		loc toplot `toplot' (``i''A, keep(3.treatment) rename(3.treatment = "```i''t'") offset(0.00) mlcolor("${p4}%0") $dotsize ms(`sym2') mfcolor("${p4}%0") ciopts( lwidth($ciwidth )  color("${p4}%0") ) )
		loc toplot `toplot' (``i''A, keep(4.treatment) rename(4.treatment = "```i''t'") offset(-0.06) mlcolor("${p6}%0") $dotsize ms(`sym3') mfcolor("${p6}%0") ciopts( lwidth($ciwidth )  color("${p6}%0") ) )
		}
		else {
				loc toplot `toplot' (``i''A, keep(2.treatment) rename(2.treatment = "```i''t'") offset(0.06) mlcolor("${p2}") $dotsize ms(`sym1') mfcolor("${p2}%50") ciopts( lwidth($ciwidth )  color("${p2}%50") ) )
		loc toplot `toplot' (``i''A, keep(3.treatment) rename(3.treatment = "```i''t'") offset(0.00) mlcolor("${p4}") $dotsize ms(`sym2') mfcolor("${p4}%50") ciopts( lwidth($ciwidth )  color("${p4}%50") ) )
		loc toplot `toplot' (``i''A, keep(4.treatment) rename(4.treatment = "```i''t'") offset(-0.06) mlcolor("${p6}") $dotsize ms(`sym3') mfcolor("${p6}%50") ciopts( lwidth($ciwidth )  color("${p6}%50") ) )
		}
		...
		if "$gname" == "support_main" 	loc xscale xscale(range(-0.005))
		else if "$gname" == "otherpols" 	{
			loc xscale xscale(range(-0.05)) xlab(-0.05(0.05)0.10)
			loc wrap 60 
		}
		else if "$gname" == "support_main_others" & "$choosecountry"=="" {
			loc xscale xscale(range(-0.05)) xlab(-0.05(0.05)0.15)
			loc wrap 60 
				}
		else if "$gname" == "support_main_others" & "$choosecountry"!="" {
			loc xscale xscale(range(-0.05)) xlab(-0.10(0.05)0.20)
			loc wrap 60 
				}
				
		else if  "$gname" != "support_main"  {
		    loc line xline(0, lcolor(black) lpattern(dash)  lwidth(thin))
		}
		else {
		    loc line 
		}
		...
		coefplot ///
		 ...
		 coeflabels(2.treatment = "Climate Impacts" 3.treatment = "Climate Policies" 4.treatment = "Both Treatments", wrap(`wrap') $coefsize ) 	///
		headings(	"Cash transfers to constrained households" = "{bf: Use carbon tax revenue for: }"   /// 
					...
					"Believes the policy would have positive econ. effects"  = "{bf: Effectiveness of the Climate Policy}"  /// 
					 ...
					 "Willing to donate to reforestation cause" = "{bf: Private Behaviors}" ///
					 "% of prize willing to donate to reforestation cause" = "{bf: Private Behaviors}" ///
				 , $headsize ) ///
				legend( order(2 "Climate Impacts" 4 "Climate Policies" 6 "Both Treatments") col(`colnum') pos(12) `commonleg2' ) ///
				`xlab' /// 
		...
		foreach i in index_trust_govt index_problem_inequality index_worried index_net_zero_feasible index_affected_subjective index_knowledge_footprint index_knowledge_fund index_knowledge_gases index_knowledge_impacts {
				* Dis Knowledge
			if strpos("`i'", "index_knowledge_footprint") 	loc `i't "Understands emissions across activities/regions" 
			if strpos("`i'", "index_knowledge_fund") 		loc `i't "Knows CC is real & caused by humans"
			if strpos("`i''", "index_knowledge_gases") 		loc `i't "Knows which gases cause CC"
			...
			/*if "`1'" =="all" & strpos("``i''", "index")==0   loc xlab = `" xlab(0(0.05)0.25, format(%4.2fc)) xsc(r(0 0.28)) "'
			else loc xlab `" xlab( , format(%4.2fc)) "' */
			loc xlab `" xlab( , format(%4.2fc)) "'
			loc toplot `toplot' (`i'A, keep(2.treatment) rename(2.treatment = "``i't'") offset(0.06) mlcolor("${p2}") $dotsize  ms(`sym1')mfcolor("${p2}%50") ciopts( lwidth($ciwidth )  color("${p2}%50") ) )
			loc toplot `toplot' (`i'A, keep(3.treatment) rename(3.treatment = "``i't'") offset(0.00) mlcolor("${p4}") $dotsize ms(`sym2')mfcolor("${p4}%50") ciopts( lwidth($ciwidth )  color("${p4}%50") ) )
			loc toplot `toplot' (`i'A, keep(4.treatment) rename(4.treatment = "``i't'") offset(-0.06) mlcolor("${p6}") $dotsize ms(`sym3')mfcolor("${p6}%50") ciopts( lwidth($ciwidth )  color("${p6}%50") ) )
		...
		coefplot ///
		 ...
		 coeflabels( , $coefsize ) 	///
		headings(	"Trusts the government" = "{bf: Trust and General Perceptions}" ///
					 "Worries about the consequences of CC" = "{bf: Views about Climate Change}" ///
					 "Understands emissions across activities/regions" =  "{bf: Climate Change Knowledge}" ///
					"Believes the policy would have positive econ. effects"  = "{bf: Effectiveness of the Climate Policy}"  /// 
					 "Believes own household would lose" = "{bf: Distributional Impacts of the Climate Policy}" ///
			...
			*graph combine gT_gen, $graphsettings_combined2  
		graph export "${gitpath}/figures/FINAL_FIGURES/Figure17a.${ft}", replace
		...
		loc toplot 
		loc count = 0
	foreach i in index_tax_positive index_tax_emissions_plus index_tax_pollution index_lose_tax_self index_lose_tax_poor index_lose_tax_rich {
		...
		coefplot ///
		 ...
		 keep(*treatment) /// 
		 $graphsettings4 xlabel( $labeloptions format(%4.1fc)) ylabel( $labeloptions nogrid ) ///
		 coeflabels(2.treatment = "Climate Impacts" 3.treatment = "Climate Policies" 4.treatment = "Both Treatments", wrap(55) $coefsize ) 	///
		headings(	"Believes the policy would have positive econ. effects"  = "{bf: Effectiveness of the Climate Policy}"  /// 
					 "Believes own household would lose" = "{bf: Distributional Impacts of the Climate Policy}" ///
				 , $headsize ) ///
				 legend(off) ///
				xtitle("Coefficients", $xops  ) ///
				title("{bf: Carbon Tax w.}" "{bf: Cash Transfers}", $xops ) /// 
				 name(gT_tax, replace) 	
		...
		foreach i in index_standard_positive index_standard_em_plus index_standard_pollution index_lose_standard_self index_lose_standard_poor index_lose_standard_rich {
			...
			loc toplot `toplot' (`i'A, keep(2.treatment) rename(2.treatment = "`i'") offset(0.06) mlcolor("${p2}")  $dotsize ms(`sym1')mfcolor("${p2}%50") ciopts( lwidth($ciwidth )  color("${p2}%50") ) )
			loc toplot `toplot' (`i'A, keep(3.treatment) rename(3.treatment = "`i'") offset(0.00) mlcolor("${p4}")  $dotsize ms(`sym2')mfcolor("${p4}%50") ciopts( lwidth($ciwidth ) color("${p4}%50") ) )
			loc toplot `toplot' (`i'A, keep(4.treatment) rename(4.treatment = "`i'") offset(-0.06) mlcolor("${p6}") $dotsize ms(`sym3')mfcolor("${p6}%50") ciopts( lwidth($ciwidth )  color("${p6}%50") ) )
		...
		coefplot ///
		 ...
		 keep(*treatment) /// 
		 $graphsettings4 xlabel( $labeloptions format(%4.1fc)) ylabel( $labeloptions nogrid ) ///
		 coeflabels(index_standard_positive = " " index_standard_em_plus = " " index_standard_pollution = " " /// 
		 index_lose_standard_self = " " index_lose_standard_poor = " " index_lose_standard_rich = " ") 	///
		headings(	"index_standard_positive"  = " "  /// 
					 "index_lose_standard_self" = " " ///
				 , labsize(small)) ///
				legend(off) ///
				xtitle("Coefficients", $xops  ) ///
				yscale(off) ///
				title("{bf: Ban on Combustion-}" "{bf:Engine Cars}", $xops  ) /// 
				fxsize(30) ///
				 name(gT_standard, replace) 
	
		// INVESTMENTS MECH. //
		loc toplot 
		loc count = 0
		
		foreach i in index_invest_positive index_invest_em_plus index_invest_pollution index_lose_invest_self index_lose_invest_poor index_lose_invest_rich {
			...
			if strpos("`i'", "index_lose_invest_self") loc `i't "Believes own household would lose"
			if strpos("`i'", "index_lose_invest_poor") loc `i't "Believes low-income earners would lose"
			if strpos("`i'", "index_lose_invest_rich") loc `i't "Believes high-income earners will lose"
		...
		coefplot ///
		 ...
		 keep(*treatment) /// 
		 $graphsettings4 xlabel( $labeloptions format(%4.1fc)) ylabel( $labeloptions nogrid ) ///
		 coeflabels(index_invest_positive = " " index_invest_em_plus = " " index_invest_pollution = " " index_lose_invest_self = " " ///
		 index_lose_invest_poor = " " index_lose_invest_rich = " ") 	///
		headings(	"index_invest_positive"  = " "  /// 
					 "index_lose_invest_self" = " " ///
				 , labsize(small)) ///
				legend(off) ///
				title("{bf: Green Infrastructure}" "{bf: Program}", $xops  ) /// 
				xtitle("Coefficients", $xops  ) ///
				yscale(off) ///
				fxsize(30) ///
				 name(gT_invest, replace) 		
				
			
		graph combine gT_tax gT_standard gT_invest, rows(1) ///
			xcommon  name(gT_combine, replace)  	
		graph export "${gitpath}/figures/FINAL_FIGURES/Figure17b.${ft}", replace
